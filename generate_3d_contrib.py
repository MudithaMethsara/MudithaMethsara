...
def generate_isometric_svg():
    ...
    try:
        with urllib.request.urlopen(req, timeout=8) as resp:
            ...
            total_contributions = data.get("total", {}).get("lastYear", sum(c["count"] for c in contribs))
    ...
    # 4. Isometric Geometry matching Image 2
    # Week axis: slopes down and to the right
    # Day axis: slopes down and to the left
    origin_x = 185
    origin_y = 150
    dx_w = 12.8
    dy_w = 3.6
    ...
    # Shading color definitions for 3D faces based on brand theme (#83CA16, #D8F89D, #10B981, #121218)
    color_palette = {
        0: { # Empty base tile
            "top": "#ECEEF0",
            "left": "#DCE0E5",
    ...
    for w, week_days in enumerate(grid):
        for d, day in enumerate(week_days):
            ...
            # Center base point of block
            bx = origin_x + (w * dx_w) + (d * dx_d)
            by = origin_y + (w * dy_w) + (d * dy_d)

            # Sorting key for painter's algorithm: depth from back to front
            # Blocks with smaller by and higher w are drawn first
            depth = by + (d * 2.0)
    ...
    for b in blocks_to_render:
        ...
        cube_path = f'''    <g class="iso-block" data-date="{b['date']}" data-count="{b['count']}">
      <!-- Left Facet -->
      <polygon points="{vx3:.2f},{vy3:.2f} {vx0:.2f},{vy0:.2f} {tx0:.2f},{ty0:.2f} {tx3:.2f},{ty3:.2f}" fill="{style['left']}" stroke="{style['left']}" stroke-width="0.3" />
      <!-- Right Facet -->
      <polygon points="{vx0:.2f},{vy0:.2f} {vx1:.2f},{vy1:.2f} {tx1:.2f},{ty1:.2f} {tx0:.2f},{ty0:.2f}" fill="{style['right']}" stroke="{style['right']}" stroke-width="0.3" />
      <!-- Top Diamond Facet -->
      <polygon points="{tx0:.2f},{ty0:.2f} {tx1:.2f},{ty1:.2f} {tx2:.2f},{ty2:.2f} {tx3:.2f},{ty3:.2f}" fill="{style['top']}" stroke="{style['top']}" stroke-width="0.3" />
    </g>'''
        cubes_svg.append(cube_path)

    cubes_str = "\n".join(cubes_svg)

    svg_content = f'''<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 960 450" width="100%" height="100%">
  <defs>
    <style>
      @import url('https://fonts.googleapis.com/css2?family=Instrument+Sans:wght@400;500;600;700&amp;family=Lora:wght@500;600&amp;display=swap');
      
      .canvas-bg {{ fill: #F7F7F8; }}
      .card-bg {{ fill: #ECEEF0; stroke: #E0E2E6; stroke-width: 1; rx: 14px; }}
      .font-sans {{ font-family: 'Instrument Sans', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; }}
      
      .header-title {{ font-family: 'Instrument Sans', sans-serif; font-size: 14.5px; font-weight: 700; fill: #121218; letter-spacing: -0.2px; }}
      .header-sub {{ font-family: 'Instrument Sans', sans-serif; font-size: 12px; font-weight: 400; fill: #61646B; }}
      
      .stat-big-lime {{ font-family: 'Instrument Sans', sans-serif; font-size: 34px; font-weight: 700; fill: #83CA16; letter-spacing: -0.8px; line-height: 1; }}
      .stat-big-green {{ font-family: 'Instrument Sans', sans-serif; font-size: 34px; font-weight: 700; fill: #165E3E; letter-spacing: -0.8px; line-height: 1; }}
      
      .stat-title {{ font-family: 'Instrument Sans', sans-serif; font-size: 13.5px; font-weight: 600; fill: #121218; }}
  ...
  <g filter="url(#cardShadow)">
    <rect x="15" y="15" width="930" height="420" class="card-bg" />
    <rect x="15" y="15" width="930" height="420" fill="url(#isoBgHatch)" rx="14" />
  </g>
  ...
  <g transform="translate(42, 45)">
    ...
    <g transform="translate(760, -8)">
      <rect x="0" y="0" width="112" height="26" rx="6" fill="#121218" />
      <circle cx="14" cy="13" r="3.5" fill="#83CA16" />
      <text x="25" y="17" font-family="'Instrument Sans', sans-serif" font-size="11" font-weight="600" fill="#FFFFFF">3D Isometric</text>
    </g>
  </g>

  <!-- ==================== TOP RIGHT STATS (Image 2 style) ==================== -->
  ...
  <!-- ==================== BOTTOM LEFT STATS (Image 2 style) ==================== -->
  <!-- Longest Streak -->
  ...
  <g transform="translate(42, 395)">
    ...
    <text x="0" y="24" class="stat-big-lime" font-size="26">{curr_streak}</text>
    <text x="24" y="16" class="stat-title">days</text>
    <text x="24" y="30" class="stat-muted">Active streak</text>
  </g>
...
{cubes_str}
  ...
  <g transform="translate(730, 410)">
    ...
    print(f"Successfully generated {output_path} matching Image 2 with {len(contribs)} days of 3D data!")
